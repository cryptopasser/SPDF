//
// Created by snakeway on 2021/3/3.
//
...
JNI_FUNC(jlong, PdfiumCore, nativeOpenDocument)(JNI_ARGS, jint fd, jstring password){
    ...
    if (!document) {
        ...
        if(errorNum == FPDF_ERR_PASSWORD) {
            jniThrowException(env, "com/snakeway/pdflibrary/PdfPasswordException",
...
JNI_FUNC(jobject, PdfiumCore, nativeGetPageSizeByIndex)(JNI_ARGS, jlong docPtr, jint pageIndex, jint dpi){
    ...
    jclass clazz = env->FindClass("com/snakeway/pdflibrary/util/Size");
...
JNI_FUNC(jobject, PdfiumCore, nativeDeviceCoordsToPage)(JNI_ARGS, jlong pagePtr, jint startX, jint startY, jint sizeX,
                                            jint sizeY, jint rotate, jdouble deviceX, jdouble deviceY) {
    FPDF_PAGE page = reinterpret_cast<FPDF_PAGE>(pagePtr);
    double pageX, pageY;
...
//load text page to find text
JNI_FUNC(jlong, PdfiumCore, nativeLoadTextPage)(JNI_ARGS,jlong pagePtr){
    FPDF_PAGE page = reinterpret_cast<FPDF_PAGE>(pagePtr);
    FPDF_TEXTPAGE textPage=FPDFText_LoadPage(page);
    if(textPage == NULL){ return  -1;}
    return reinterpret_cast<jlong>(textPage);
}

//close text page
JNI_FUNC(void, PdfiumCore, nativeCloseTextPage)(JNI_ARGS,jlong pageTextPtr){
    FPDF_TEXTPAGE page = reinterpret_cast<FPDF_TEXTPAGE>(pageTextPtr);
    FPDFText_ClosePage(page);
}

//get text count
JNI_FUNC(jint, PdfiumCore, nativePageTextCount)(JNI_ARGS,jlong pageTextPtr){
    FPDF_TEXTPAGE page = reinterpret_cast<FPDF_TEXTPAGE>(pageTextPtr);
    int count=FPDFText_CountChars(page);
    return reinterpret_cast<jint>(count);
}

//get char index by position
JNI_FUNC(jint, PdfiumCore, nativeGetCharIndexAtPos)(JNI_ARGS,jlong pageTextPtr,jdouble x,jdouble y,jdouble xTolerance,jdouble yToerance){
    FPDF_TEXTPAGE page = reinterpret_cast<FPDF_TEXTPAGE>(pageTextPtr);
    int index=FPDFText_GetCharIndexAtPos(page,x,y,xTolerance,yToerance);
    return reinterpret_cast<jint>(index);
}

//get text rect by char index
JNI_FUNC(jobject, PdfiumCore, nativeTextRect)(JNI_ARGS, jlong pageTextPtr, jint index) {
    FPDF_TEXTPAGE page = reinterpret_cast<FPDF_TEXTPAGE>(pageTextPtr);
    double left=0;
    double right=0;
    double bottom=0;
    double top=0;
    FPDFText_GetCharBox(page,index,&left,&right,&bottom,&top);
    jclass clazz = env->FindClass("android/graphics/RectF");
    jmethodID constructorID = env->GetMethodID(clazz, "<init>", "(FFFF)V");
...
JNI_FUNC(jint, PdfiumCore, nativeSearchTextUnicode)(JNI_ARGS, jlong pageTextPtr,jint index){
    FPDF_TEXTPAGE page = reinterpret_cast<FPDF_TEXTPAGE>(pageTextPtr);
    unsigned int unicode=FPDFText_GetUnicode(page,index);
    return jint(unicode);
}
