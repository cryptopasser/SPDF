 ...
 * @author snakeway
 */
public class MarkRectEraser implements Eraser {
    //橡皮的宽度
    private int width;
    //橡皮的高度
    private int height;
     ...
     */
    public MarkRectEraser(int width, int height, boolean zoomable) {
        this.width = width;
        this.height = height;
    ...
    public BaseAnnotation erase(List<BaseAnnotation> annotations, float scale, float viewZoom, PDFView pdfView) {
        ...
        RectF rect = new RectF(x - widthSpace, y - heightSpace, x + widthSpace, y + heightSpace);
        for (BaseAnnotation annotation : annotations) {
            if (annotation instanceof MarkAnnotation) {
                ...
                for (RectF rectItem : a.data) {
                    RectF f = CoordinateUtils.toPdfPointCoordinateDesc(pdfView, a.page, rectItem);
                    if (RectF.intersects(f, rect)) {
                        return annotation;
                    }
                }
            }
