...
import static com.snakeway.pdfviewer.util.Constants.Cache.CACHE_SIZE;
import static com.snakeway.pdfviewer.util.Constants.PRELOAD_OFFSET;
    ...
    private List<RenderRange> getRenderRangeList(float firstXOffset, float firstYOffset, float lastXOffset, float lastYOffset) {
        ...
        for (int page = firstPage; page <= lastPage; page++) {
            ...
            if (pdfView.isSwipeVertical()) {
                ...
                range.rightBottom.row = MathUtils.ceil(Math.abs(pageLastYOffset - pageOffset) / rowHeight);
                range.rightBottom.col = MathUtils.floor(MathUtils.min(pageLastXOffset - secondaryOffset, 0) / colWidth);
            } else {
                range.leftTop.col = MathUtils.floor(MathUtils.min(pageFirstXOffset - pageOffset, 0) / colWidth);
                range.leftTop.row = MathUtils.floor(MathUtils.min(pageFirstYOffset - secondaryOffset, 0) / rowHeight);

                range.rightBottom.col = MathUtils.floor(Math.abs(pageLastXOffset - pageOffset) / colWidth);
