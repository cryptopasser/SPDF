...
import static com.snakeway.pdfviewer.util.Constants.Pinch.MAXIMUM_ZOOM;
import static com.snakeway.pdfviewer.util.Constants.Pinch.MINIMUM_ZOOM;
...
class DragPinchManager implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener, ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    private void startPageFling(MotionEvent downEvent, MotionEvent ev, float velocityX, float velocityY) {
        ...
        SnapEdge edge = pdfView.isAutoFillWhiteSpace() ? SnapEdge.CENTER : pdfView.findSnapEdge(targetPage);
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        if (pdfView.isAutoFillWhiteSpace() && (pdfView.doZooming || pdfView.doMoving)) {
            return false;
        }
        if(pdfView.isSingleZoom()){
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (pdfView.isZooming() || pdfView.isSwipeEnabled()) {
            pdfView.moveRelativeToWithWhiteSpaceTouchLimitCheck(-distanceX, -distanceY);
        }
        if (!scaling || pdfView.doRenderDuringScale()) {
            if (pdfView.isAutoFillWhiteSpace()) {
                pdfView.loadPageByOffsetWithAutoFillCheck();
            } else {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (pdfView.isPageFlingEnabled() || !pdfView.isSwipeVertical()) {//if is not vertical only can fling one page
            boolean isHorizontalTouch = Math.abs(velocityX) > Math.abs(velocityY);
            if (pdfView.pageFillsScreen()) {
                if (pdfView.checkWhiteSpaceTouchLimit()) {
                    if (isHorizontalTouch && (!pdfView.isLastPage() || (pdfView.isLastPage() && velocityX > 0)) && (!pdfView.isFirstPage() || (pdfView.isFirstPage() && velocityX < 0))) {
                        startPageFling(e1, e2, velocityX, velocityY);
                    }else{
                        if(!pdfView.isDoZooming()&&!pdfView.isDoMoving()&&!pdfView.isDoFlinging()) {
                            onBoundedFling(0, velocityY);
                        }
                    }
                    return false;
                }
    ...
    private void onBoundedFling(float velocityX, float velocityY) {
        ...
        animationManager.startFlingAnimation(xOffset, yOffset, (int) velocityX, (int) velocityY,
                (int) Math.min(minX,maxX), (int) Math.max(minX,maxX), (int) Math.min(minY,maxY), (int) Math.max(minY,maxY));
    }
